...
def discover_patterns_stumpy_mixed(
    ...
    The function relies on FAISS to determine the true medoid regardless of the
    column name.
    """
...
def discover_patterns_mstump_mixed(
    ...
    Steps
    -----
    1. Compute the multi-dimensional matrix profile ``P`` and indices ``I``.
    2. Select relevant dimensions using MDL.
    3. Optionally extract motifs from the reduced data.
    4. Filter discords from the full profile.
    5. Return aligned motifs, discord indices and the centered profile.
    """
