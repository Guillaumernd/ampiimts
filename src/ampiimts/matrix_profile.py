"""Matrix profile computation utilities."""
...
def matrix_profile_process(
    ...
    """Internal helper to compute matrix profile for a single DataFrame."""
    ...
    # ==== MULTIVARIATE ====
        ...
        motif_distances, motif_indices, motif_subspaces, motif_mdls = (
            sp.mmotifs(df, P, I, max_motifs=3, max_matches=5, normalize=False)
        )
        ...
        df_profile = pd.DataFrame(
            P.T, columns=[f"value_{col}" for col in df.columns]
        )
        df_profile.index = df.index[center_indices]

        df_index = pd.DataFrame(
            I.T, columns=[f"index_{col}" for col in df.columns]
        )
